...
void add_two_digits (int* chiffresIn,int taille,int* chiffresOut) {
    ...
    for (int i = 0; i < taille - 1; i += 2) {
        chiffresOut[j] = chiffresIn[i] + chiffresIn[i+1];
        j++;
    }
    if (taille % 2 == 1) {
        chiffresOut[j] = chiffresIn[taille-1];
    }
}
...
void numbers_to_digits (int* numbers, int taille) {
    ...
    for (int i = 0 ; i < taille ; i++) {
        if (numbers[i] > 9) {
            ...
            int dernier_chiffre = original % 10;
            int premier_chiffre = original / 10;
            numbers[i] = premier_chiffre + dernier_chiffre;
        }
    }
}
...
float compatibility(char* prenom1, char* prenom2) {
    // TODO: à completer 
    int score1 = score(prenom1);
    int score2 = score(prenom2);
    int difference = abs(score1 - score2);
    float compatibilite = 100.0 - (difference * 12.5);
    
    return compatibilite / 100.0;
}
...
int main (int argc,char** argv)
{
    ...
    scanf(" %ms", &prenom2);
    //TODO: à compléter
    printf("La compatibilité de %s et %s est de %.3f %%\n",prenom1, prenom2, compatibility(prenom1, prenom2) * 100);
